...
<table style="width: 100%" id="col1">
	...
	<c:forEach items="${requestScope.list}" var="order" varStatus="status">
      <tr>
        ...
        <td height="25"><a href='${pageContext.request.contextPath}/p/orderDetail/${order.subNumber}' target="_blank">${order.subNumber}</a></td>
        ...
        <c:if test="${'C2C' == applicationScope.BUSINESS_MODE}">
        <td height="25"><a href="<ls:domain shopName='${order.shopName}' />" target="_blank">${order.shopName}</a></td>
        </c:if>
        ...
        <a href='${pageContext.request.contextPath}/p/orderDetail/${order.subNumber}' target="_blank"><fmt:message key="payment.hint"/></a> 
        </c:if>
