...
<br>
<form action="${root}/businessAction/loginHistorySum.c" id="form1">
<input type="hidden" id="curPageNO" name="curPageNO" value="${login.curPageNO}">
			...
			开始时间 (格式yyyy-MM-dd)：
			<input type="text" name="startTime" id="startTime" maxlength="50" value='<fmt:formatDate value="${login.startTime}" pattern="yyyy-MM-dd"/>' size="15" class="calendar"/>
			结束时间 (格式yyyy-MM-dd)：
			<input type="text" name="endTime" id="endTime" maxlength="50" value='<fmt:formatDate value="${login.endTime}" pattern="yyyy-MM-dd"/>' size="15" class="calendar"/>
			<html:submit value="搜索" property="Submit" />
	</div>

<table width="100%" border="0" align="center" class="tableBorder">
  <tr>
    <th height="29" colspan="7" scope="col">用户登录历史列表次数</th>
  </tr>
  <c:if test="${list != null }">
  <tr>
    <td align="center" class="forumRow" width="5%">顺序</td>
    <td align="center" class="forumRow" width="6%">用户名</td>
    <td align="center" class="forumRow" width="12%">登录次数</td>

    </tr>
  <c:forEach items="${requestScope.list}" var="login">
  <tr>
    <td align="center" class="forumRow"><%=offset++%></td>
    <td align="center" class="forumRow">${login[0]}</td>
    <td align="center" class="forumRow">${login[1]}</td>
    </tr>
  </c:forEach>
  </c:if>
</table>
