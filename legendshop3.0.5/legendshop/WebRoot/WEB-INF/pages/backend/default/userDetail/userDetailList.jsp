      ...
      <auth:auth ifAnyGranted="F_OPERATOR">
	      ...
	      <c:choose>
	      	<c:when test="${item.shopId != null}">
	      		<a href='${pageContext.request.contextPath}/admin/shopDetail/load/${item.shopId}${applicationScope.WEB_SUFFIX}?userName=${item.userName}' title="修改用户${item.userName}当前商城"><img alt="修改用户当前商城" src="<ls:templateResource item='/common/default/images/grid_edit.png'/> "></a>
	      	</c:when>
	      	<c:otherwise>
	      		<c:if test="${supportOpenShop == 'true' && 'C2C' == applicationScope.BUSINESS_MODE}">
	      			<a href='${pageContext.request.contextPath}/admin/shopDetail/load/0${applicationScope.WEB_SUFFIX}?userName=${item.userName}'><img alt="为用户${item.userName}增加商城" src="${pageContext.request.contextPath}/common/default/images/grid_add.png"></a>
	      		</c:if>
	      	</c:otherwise>
	      </c:choose>
	      </c:if>
  	 <a href='javascript:confirmDelete("${item.userId}","${item.userName}")' title="删除用户${item.userName}所有信息"><img alt="删除用户${item.userName}所有信息" src="<ls:templateResource item='/common/default/images/grid_delete.png'/> "></a>
	 </display:column>
      </auth:auth>
    </display:table> 
            <auth:auth ifAnyGranted="F_OPERATOR">
