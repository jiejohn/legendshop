...
<%
    ...
    <thead>
		...
		<input type="hidden" name="curPageNO" id="curPageNO" value="${curPageNO}"/>
			...
			商城名称&nbsp;<input type="text" name="search" maxlength="50" value="<%=request.getAttribute("search")%>" />
			</auth:auth>
...
<div align="center">
    ...
    <display:table name="list" requestURI="/admin/userComment/query${applicationScope.WEB_SUFFIX}" id="item"
      ...
      <display:column title="添加时间" property="addtime" sortable="true" sortName="addtime"></display:column>
     ...
     <auth:auth ifAnyGranted="F_VIEW_ALL_DATA">
     <display:column title="商城名称" property="toUserName" sortable="true" sortName="toUserName"></display:column>
     </auth:auth>
...
<!--
  ...
  function openWindows(id) {
 		 var win = window.open('${pageContext.request.contextPath}/admin/userComment/update/'+id + '${applicationScope.WEB_SUFFIX}','回复用户消息','height=350px,width=650px, scroll=no, status=yes,top=250,left=250');	
		 win.focus() ;
		 } 
