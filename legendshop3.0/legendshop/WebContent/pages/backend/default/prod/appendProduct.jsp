...
<html>
	<head>
       ...
       function save(obj){
          ...
          CommonService.saveProdItem(idJson,nameJson,'${prodId}','${sessionScope.SPRING_SECURITY_LAST_USERNAME}',function(data){
              if(data==null){
                 alert("操作成功");
                 window.close();               
              }else{
	...
	function loadDataByName(){
		 	var name = "%"+document.getElementById("selectname").value+"%";
		 	 CommonService.getUsableProductItemByName('${prodId}', '${sessionScope.SPRING_SECURITY_LAST_USERNAME}', name,function(dataArray){
                try{
       ...
       function loadData(){
          //alert("loadData " + '${prodId}' +" " + '${sessionScope.SPRING_SECURITY_LAST_USERNAME}');
	       //未有选择
           CommonService.getUsableProductItem('${prodId}','${sessionScope.SPRING_SECURITY_LAST_USERNAME}',function(dataArray){
               try{
                 for(var i=0;i<dataArray.length ;i++){        
                     var data = dataArray[i] ;                                   
					 dwr.util.addOptions("Select1", [{ value:data.key, disp : data.value }], "value", "disp");
           ...
           //已经选择的
           CommonService.getUsedProductItem('${prodId}','${sessionScope.SPRING_SECURITY_LAST_USERNAME}',function(dataArray){
                try{
                 for(var i=0;i<dataArray.length ;i++){
                     var data = dataArray[i] ;
	...
	<body scroll="no" onLoad="loadData()">
		<table width="100%" height="100%" cellpadding="1" cellspacing="1">
			<tr>
				<td><br>
				&nbsp;&nbsp;<img src="${pageContext.request.contextPath}/plugins/theme/skin/images/frame/menusearch.gif">商品名称										
