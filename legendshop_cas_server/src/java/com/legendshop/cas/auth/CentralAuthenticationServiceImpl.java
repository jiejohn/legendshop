 ...
 * </ul>
 * <p>
 * We extend the grantServiceTicket action to check if the authenticated user
 * has the right to use the application(service).
 * </p>
 * 
...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
	...
	private final Log log = LogFactory.getLog(CentralAuthenticationServiceImpl.class);
	...
	@Statistic(name = "DESTROY_TICKET_GRANTING_TICKET", requiredPrecision = { Precision.DAY, Precision.MINUTE,
			Precision.HOUR })
	...
	public void destroyTicketGrantingTicket(final String ticketGrantingTicketId) {
		...
		final TicketGrantingTicket ticket = (TicketGrantingTicket) this.ticketRegistry.getTicket(
		...
		if (log.isDebugEnabled()) {
			log.debug("Ticket found. To do Expiring and then deleting.");
		}

		try {
			ticket.expire();
		} catch (Exception e) {
			log.error("Failed to expire the ticket, error: " + e.getMessage());
		}

		this.ticketRegistry.deleteTicket(ticketGrantingTicketId);

		if (log.isDebugEnabled()) {
			log.debug("Finished to expire and delete this ticket.");
		}
	}
	...
	public String grantServiceTicket(final String ticketGrantingTicketId, final Service service,
		...
		ticketGrantingTicket = (TicketGrantingTicket) this.ticketRegistry.getTicket(ticketGrantingTicketId,
		...
		if (credentials != null) {
			try {
				...
				final Principal originalPrincipal = ticketGrantingTicket.getAuthentication().getPrincipal();
		...
		final ServiceTicket serviceTicket = ticketGrantingTicket.grantServiceTicket(
				serviceTicketUniqueTicketIdGenerator.getNewTicketId(ServiceTicket.PREFIX), service,
				this.serviceTicketExpirationPolicy, credentials != null);

		this.serviceTicketRegistry.addTicket(serviceTicket);

		if (log.isInfoEnabled()) {
			log.info("Granted service ticket [" + serviceTicket.getId() + "] for service [" + service.getId()
					+ "] for user [" + serviceTicket.getGrantingTicket().getAuthentication().getPrincipal().getId()
					+ "]");
		}
	...
	public Assertion validateServiceTicket(final String serviceTicketId, final Service service) throws TicketException {
		...
		final ServiceTicket serviceTicket = (ServiceTicket) this.serviceTicketRegistry.getTicket(serviceTicketId,
				ServiceTicket.class);
		...
		try {
				...
				if (!serviceTicket.isValidFor(service)) {
					if (log.isErrorEnabled()) {
						log.error("ServiceTicket [" + serviceTicketId + "] with service ["
								+ serviceTicket.getService().getId() + " does not match supplied service [" + service
								+ "]");
					}
			...
			final Authentication authentication = serviceTicket.getGrantingTicket().getChainedAuthentications()
					.get(authenticationChainSize - 1);
			final Principal principal = authentication.getPrincipal();
			final String principalId = registeredService.isAnonymousAccess() ? this.persistentIdGenerator.generate(
					principal, serviceTicket.getService()) : principal.getId();
			...
			Assertion immutableAssertion = new ImmutableAssertionImpl(authentications, serviceTicket.getService(),
					serviceTicket.isFromNewLogin());

			if (log.isDebugEnabled()) {
				log.debug("validateServiceTicket success, assertion: " + immutableAssertion);
			}
	...
	@Statistic(name = "CREATE_TICKET_GRANTING_TICKET", requiredPrecision = { Precision.DAY, Precision.MINUTE,
			Precision.HOUR })
	@Transactional(readOnly = false)
